...
箭头右侧会话等待左侧会话。根 holder 仍然只有一个会话/事务，但为每条分支持有
一个不同的根行；这样各分支第一层都直接等待同一个 holder 事务，同时避免多个
UPDATE 在同一根行上被数据库 tuple-lock 队列串行化。每个分支节点先在独立事务中
更新并持有自己的唯一 `lock_targets` 行，再异步更新上游节点持有的行：第一层更新
本分支根行，第二层更新第一层持有的行，依次类推。全部会话使用包含分支号和层号
的唯一 tagged session 名称。构造结果应有 `sessions-1` 条受控等待边，观测到的
最大层数应等于 `chain_depth`。

分支和行号分配必须是确定性的纯逻辑，以便在打开连接之前完成边界检查和单元
测试。最大深度为 5；所需行数为 waiter 数加分支根行数。可用行数不足时必须明确
失败，不能复用行而改变拓扑。
