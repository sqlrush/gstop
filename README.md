...
internal/
  ...
  app/                主循环 + 可暂停刷新线程 + 键位状态机(s/m/e 子模式)
...
./scripts/build.sh v1.4.0
#  -> dist/gstop_v1.4.0_linux_arm64_YYYYMMDD.tar.gz
#  -> dist/gstop_v1.4.0_linux_amd64_YYYYMMDD.tar.gz
```
...
`r` 实时事件 / `c` 累计事件 / `s` 会话选择 / `m` 内存大盘 / `e` 应急面板 / `q` 退出。
