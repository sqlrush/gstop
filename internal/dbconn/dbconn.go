...
// maxPoolConns intentionally preserves the original gstop gateway semantics:
// every resident monitor shares one physical database connection. In
// particular, the openGauss connector/thread-pool combination must not have
// multiple result streams active through one gateway because that can associate
// a response with the wrong monitor under pressure. database/sql still queues
// callers with context-aware cancellation, and separate DB gateways remain
// independent.
...
func (d *DB) operationContext(parents ...context.Context) (context.Context, context.CancelFunc) {
	...
	ctx, cancelTimeout := context.WithTimeout(base, config.CollectTimeout(d.cfg))
	return ctx, func() {
		stopParent()
		cancelTimeout()
		cancelBase()
...
func (d *DB) ensure(ctx context.Context) *sql.DB {
	...
	if d.pool == nil {
		...
		// The Python implementation shared one psycopg connection across the
		// monitor threads. Keep one physical connection here as well; database/sql
		// safely serialises concurrent callers and honours their contexts while
		// they wait.
