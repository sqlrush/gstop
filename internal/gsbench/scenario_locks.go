...
func (s *LockScenario) Code() ScenarioCode {
	return 0
}
func (s *LockScenario) Name() string { return "lock_storm" }

func literalRowUpdate(schema, table string, id int) string {
	return "UPDATE " + schema + "." + table +
		" SET value=value+1 WHERE id=" + strconv.Itoa(id)
}
...
func (s *LockScenario) Prepare(ctx context.Context, rt *Runtime) error {
	...
	for i, role := range plan.Blockers {
		...
		switch role.Kind {
		case "row":
			lockSQL = literalRowUpdate(rt.Config.Data.Schema, "lock_targets", role.Row)
		case "table":
	...
	if plan.Deadlock {
		for i, row := range []int{100, 101} {
			...
			if _, err := tx.ExecContext(ctx, literalRowUpdate(rt.Config.Data.Schema, "lock_targets", row)); err != nil {
				tx.Rollback()
				conn.Close()
				return err
			}
...
func (s *LockScenario) Ramp(ctx context.Context, rt *Runtime) error {
	...
	for i := 1; i < rowBlockers; i++ {
		...
		query := literalRowUpdate(rt.Config.Data.Schema, "lock_targets", row)
		s.wg.Add(1)
		go func() { defer s.wg.Done(); _, _ = tx.ExecContext(s.ctx, query) }()
	}
	for i, role := range s.plan.Waiters {
		...
		case "row":
			...
			s.transactions = append(s.transactions, tx)
			query := literalRowUpdate(rt.Config.Data.Schema, "lock_targets", role.ParentRow)
			go func() { defer s.wg.Done(); _, _ = tx.ExecContext(s.ctx, query) }()
		case "table":
			go func() {
				defer s.wg.Done()
	...
	if s.plan.Deadlock {
		...
		s.wg.Add(2)
		firstSQL := literalRowUpdate(rt.Config.Data.Schema, "lock_targets", 101)
		secondSQL := literalRowUpdate(rt.Config.Data.Schema, "lock_targets", 100)
		go func() { defer s.wg.Done(); _, _ = first.ExecContext(s.ctx, firstSQL) }()
		go func() { defer s.wg.Done(); _, _ = second.ExecContext(s.ctx, secondSQL) }()
	}
