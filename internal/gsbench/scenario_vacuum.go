...
func (s *VacuumScenario) Code() ScenarioCode {
	return 801
}
func (s *VacuumScenario) Name() string { return "vacuum_pressure" }
func (s *VacuumScenario) Strategy() string {
	return "vacuum_foreground_slowdown"
}
func (s *VacuumScenario) Prepare(ctx context.Context, rt *Runtime) error {
	...
	mutation := Mutation{RunID: rt.RunID, ScenarioCode: s.Code(), Kind: "vacuum_churn", Target: rt.Config.Data.Schema + ".vacuum_targets", ForwardSQL: "UPDATE " + rt.Config.Data.Schema + ".vacuum_targets SET version=version+1,payload=payload||'x',updated_at=current_timestamp", InverseSQL: "UPDATE " + rt.Config.Data.Schema + ".vacuum_targets SET version=0,payload=repeat('v',900),updated_at=current_timestamp", VerifySQL: "SELECT count(*) FROM " + rt.Config.Data.Schema + ".vacuum_targets WHERE version<>0", VerifyValue: "0"}
...
func (s *VacuumScenario) Restore(context.Context, *Runtime) error { return nil }
func measureVacuumForeground(ctx context.Context, db *Database, schema string) (time.Duration, error) {
	started := time.Now()
	var value string
	err := db.Scan(ctx, vacuumForegroundSQL(schema), nil, &value)
	return time.Since(started), err
}

func vacuumForegroundSQL(schema string) string {
	return "SELECT sum(length(payload)) FROM " + schema + ".vacuum_targets"
}
