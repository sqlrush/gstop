...
func TPStatements(schema string, id, orderID int64, balance float64) []string {
	...
	return []string{
		"SELECT balance FROM " + schema + ".accounts WHERE id=" + idText,
		"UPDATE " + schema + ".accounts SET balance=balance+1,updated_at=current_timestamp WHERE id=" + idText,
		"INSERT INTO " + schema + ".orders(id,customer_id,status,amount,created_at) VALUES(" +
			orderText + "," + idText + ",0," + balanceText + ",current_timestamp)",
	}
}
