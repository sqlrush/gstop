...
// Sync invalidates tcell's physical-screen cache and repaints the complete
// frame. The main dashboard uses it so a transient terminal write failure
// cannot leave cells marked clean while they are absent from the terminal.
func (s *Screen) Sync() { s.scr.Sync() }
