...
// CommandRunner is the optional OS boundary used for low-frequency cm_ctl
// topology enrichment.
type CommandRunner interface {
	Run(command string, check bool) (string, bool)
}
...
type Collector struct {
	...
	lastStatements       map[int64]StatementSample
...
func NewCollector(cfg *config.Config, db Queryer, logger *logging.Logger, memoryGate MemoryRefreshGate, commands CommandRunner) *Collector {
	now := time.Now
	memoryEnabled := cfg.GetBool("main.dynamic_mem_enable", false) && cfg.GetInt("main.mem_interval", 30) > 0
	return &Collector{
		cfg: cfg, db: db, logger: logger, now: now, memoryGate: memoryGate, commands: commands,
		snapshot: Snapshot{
			StartedAt:     now(),
...
func (c *Collector) RefreshFast() {
	...
	refreshedAt := c.now()
	var statementRows, activeRows, waitRows, cpuRows, lockRows, replicationRows []dbconn.Row
	...
	go func() {
		defer queries.Done()
		lockRows = c.db.Query(lockHealthQuery)
	}()
	go func() {
		defer queries.Done()
		replicationRows = c.db.Query(replicationQuery)
		if replicationRows == nil {
			replicationRows = c.db.Query(replicationFallbackQuery)
		}
	}()
	queries.Wait()
	...
	locks, lockOK := parseLockHealth(lockRows)
	replication, replicationOK := parseReplicationHealth(replicationRows)
	if !statementOK {
	...
	if statementOK {
		if !c.statementBaselineSet {
			...
			c.statementBaseline = rebaseStatements(c.statementBaseline, statements, c.lastStatements)
		}
		if c.lastStatements == nil {
			c.lastStatements = make(map[int64]StatementSample, len(statements))
		}
		for _, statement := range statements {
			c.lastStatements[statement.SQLID] = statement
		}
		c.snapshot.AverageSQL, c.snapshot.ExecutionSQL = BuildSQLMetrics(statements, c.statementBaseline, active, refreshedAt)
	} else {
	...
	if lockOK {
		c.snapshot.Lock = locks
		c.snapshot.LockError = ""
	} else {
		c.snapshot.Lock = LockHealth{}
		c.snapshot.LockError = "锁查询失败或超时"
	}
	if replicationOK {
		c.snapshot.Replication = replication
		c.snapshot.ReplicationError = ""
	} else {
		c.snapshot.Replication = ReplicationHealth{}
		c.snapshot.ReplicationError = "主备同步查询失败或无权限"
	}
...
func (c *Collector) startMemoryRefresh(capturedAt time.Time, active []ActiveSQL) {
	...
	c.mu.Lock()
	due := c.lastMemoryRefresh.IsZero() || capturedAt.Sub(c.lastMemoryRefresh) >= interval
	...
	if due && c.beginWork(&c.memoryBusy) {
		c.lastMemoryRefresh = capturedAt
		go c.refreshMemory(capturedAt, append([]ActiveSQL(nil), active...))
	}
	c.mu.Unlock()
}

func (c *Collector) refreshMemory(capturedAt time.Time, active []ActiveSQL) {
	...
	c.mu.Lock()
	c.snapshot.MemorySQL = BuildMemoryMetrics(active, capturedAt)
	c.snapshot.MemoryRefreshedAt = c.now()
...
func (c *Collector) refreshSlow(refreshedAt time.Time) {
	...
	go func() {
		defer queries.Done()
		clusterRows = c.db.Query(clusterTopologyQuery)
	}()
	go func() {
		defer queries.Done()
		if c.commands != nil {
			cmOutput, cmOK = c.commands.Run(clusterCMCommand, false)
		}
	}()
	queries.Wait()
	analyze, analyzeErrors := parseAnalyzeResults(analyzeResults)
	indexes, indexErrors := parseIndexResults(indexResults)
	clusterNodes, sqlAvailable := parseClusterNodes(clusterRows)
	cmComponents := parseCMStatus(cmOutput)
	cmAvailable := cmOK && len(cmComponents) > 0
	...
	c.snapshot.Cluster = ClusterHealth{
		SQLAvailable: sqlAvailable,
		CMAvailable:  cmAvailable,
		Nodes:        clusterNodes,
		Components:   cmComponents,
	}
...
func parseStatements(rows []dbconn.Row) ([]StatementSample, bool) {
	...
	for _, row := range rows {
		...
		databaseIndex, userIndex, callsIndex, dbTimeIndex, queryIndex := 1, 2, 3, 4, 5
		if len(row) < 6 {
			// Preserve compatibility with older collectors and test fixtures.
			databaseIndex, userIndex = -1, -1
			callsIndex, dbTimeIndex, queryIndex = 1, 2, 3
		}
		calls, _ := row.Int(callsIndex)
		dbTime, _ := row.Float(dbTimeIndex)
		sample := StatementSample{
			SQLID: id, Calls: calls, DBTimeUS: dbTime, Query: row.Str(queryIndex),
		}
		if databaseIndex >= 0 {
			sample.Databases = sortedUniqueStrings([]string{row.Str(databaseIndex)})
			sample.Users = sortedUniqueStrings([]string{row.Str(userIndex)})
		}
		if index, exists := positions[id]; exists {
			out[index] = mergeStatementSample(out[index], sample)
			continue
		}
...
func parseActive(rows []dbconn.Row) ([]ActiveSQL, bool) {
	...
	for _, row := range rows {
		...
		if len(row) >= 7 {
			active.Database = row.Str(5)
			active.User = row.Str(6)
		}
		if len(row) >= 8 {
			active.QueryStart, _ = row.Time(7)
		}
...
func rebaseStatements(baseline, current []StatementSample, previous map[int64]StatementSample) []StatementSample {
	...
	for _, row := range current {
		last, seen := previous[row.SQLID]
		if !seen || (row.Calls >= last.Calls && row.DBTimeUS >= last.DBTimeUS) {
			continue
		}
